...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id_libro'])) {
    ...
    try {
        ...
        header("Location: dettaglio_libro.php?id={$id_libro}&prestito=success&scadenza=" . urlencode(date('d/m/Y', strtotime($data_scadenza))));
        exit;
        ...
        $error = $e->getMessage();
        header("Location: dettaglio_libro.php?id={$id_libro}&error=" . urlencode($error));
        exit;
    }
}
