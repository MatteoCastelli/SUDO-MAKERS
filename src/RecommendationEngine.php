...
class RecommendationEngine
{
     ...
     */
    /**
     * Traccia una interazione dell'utente con un libro.
     *
     * @param int $idUtente
     * @param int $idLibro
     * @param string $tipo Tipo di interazione (click, view, prenotazione, rating, like, dislike)
     * @param string|null $fonte (opzionale) Fonte o contesto della interazione
     * @param int|null $durata (opzionale) Durata in secondi dell'interazione, se rilevante
     *
     * @return bool true se successo, false altrimenti
     */
    public function trackInteraction(int $idUtente, int $idLibro, string $tipo, ?string $fonte = null, ?int $durata = null): bool
    {
        // Valid types
        $validTypes = ['click', 'view', 'prenotazione', 'rating', 'like', 'dislike'];
        if (!in_array($tipo, $validTypes)) {
            return false; // tipo non valido
        }

        // Qui puoi fare operazioni diverse a seconda del tipo
        // Ad esempio, per rating potresti salvare un valore extra
        // Per prenotazione potresti aggiornare disponibilità o stato libro ecc.

        try {
            $stmt = $this->pdo->prepare("
                INSERT INTO interazioni_utente_libro 
                    (id_utente, id_libro, tipo_interazione, fonte, durata, data_interazione)
                VALUES 
                    (:id_utente, :id_libro, :tipo, :fonte, :durata, NOW())
            ");
            $stmt->bindValue(':id_utente', $idUtente, PDO::PARAM_INT);
            $stmt->bindValue(':id_libro', $idLibro, PDO::PARAM_INT);
            $stmt->bindValue(':tipo', $tipo, PDO::PARAM_STR);
            $stmt->bindValue(':fonte', $fonte, $fonte ? PDO::PARAM_STR : PDO::PARAM_NULL);
            $stmt->bindValue(':durata', $durata, $durata ? PDO::PARAM_INT : PDO::PARAM_NULL);

            $stmt->execute();

            // Esempio: aggiorna rating medio se tipo = rating (qui valore rating immagazzinato in durata)
            if ($tipo === 'rating' && $durata !== null) {
                $this->updateAverageRating($idLibro);
            }

            // Altri aggiornamenti o logiche specifiche...
    ...
    private function updateAverageRating(int $idLibro)
    {
        // Calcola rating medio da tutte le interazioni tipo rating per questo libro
        $stmt = $this->pdo->prepare("
            SELECT AVG(durata) as avg_rating 
            FROM interazioni_utente_libro 
            WHERE id_libro = :id_libro AND tipo_interazione = 'rating'
        ");
        $stmt->bindValue(':id_libro', $idLibro, PDO::PARAM_INT);
        $stmt->execute();
        $avg = $stmt->fetchColumn();

        // Aggiorna tabella libro con rating medio
        $update = $this->pdo->prepare("
            UPDATE libro SET rating_medio = :avg_rating WHERE id_libro = :id_libro
        ");
        $update->bindValue(':avg_rating', $avg);
        $update->bindValue(':id_libro', $idLibro, PDO::PARAM_INT);
        $update->execute();
    }
    public function getRecommendedBooks(int $idUtente, int $limit = 10): array
    {
        // Pesi per tipo
        $weights = [
            'click' => 1,
        ...
        // Query per sommare punteggi pesati con decay temporale
        $stmt = $this->pdo->prepare("
        SELECT il.id_libro, 
            SUM(
                (CASE il.tipo_interazione
                    WHEN 'click' THEN :click_weight
                    WHEN 'view' THEN :view_weight
                    WHEN 'prenotazione' THEN :prenotazione_weight
                    WHEN 'rating' THEN :rating_weight * il.durata
                    WHEN 'like' THEN :like_weight
                    WHEN 'dislike' THEN :dislike_weight
                    ELSE 0
                END)
                * EXP(-TIMESTAMPDIFF(DAY, il.data_interazione, NOW()) / 90)  -- decay 90 giorni
            ) as punteggio
        FROM interazioni_utente_libro il
        WHERE il.id_utente = :id_utente
        GROUP BY il.id_libro
        ORDER BY punteggio DESC
        LIMIT :limit
    ");
        $stmt->bindValue(':click_weight', $weights['click']);
        $stmt->bindValue(':view_weight', $weights['view']);
        $stmt->bindValue(':prenotazione_weight', $weights['prenotazione']);
        $stmt->bindValue(':rating_weight', $weights['rating']);
        $stmt->bindValue(':like_weight', $weights['like']);
        $stmt->bindValue(':dislike_weight', $weights['dislike']);
        $stmt->bindValue(':id_utente', $idUtente, \PDO::PARAM_INT);
    ...
    public function updateTrendingStats()
    {
        ...
        $stmt = $this->pdo->query("
            SELECT 
                ...
                COUNT(DISTINCT CASE 
                    ...
                    THEN p.id_prestito END) as prestiti_7d,
                COUNT(DISTINCT CASE 
                    WHEN p.data_prestito >= DATE_SUB(NOW(), INTERVAL 30 DAY) 
                    THEN p.id_prestito END) as prestiti_30d,
                COUNT(DISTINCT CASE 
                    WHEN i.data_interazione >= DATE_SUB(NOW(), INTERVAL 7 DAY) 
                    THEN i.id_interazione END) as click_7d,
        ...
        foreach ($stats as $stat) {
            // Calcola trend score
            $time_decay = 0.7; // peso maggiore ai prestiti recenti
            $trend_score =
    ...
    /**
     * Ottieni libri raccomandati per l'utente
     */
    public function getRecommendations($id_utente, $limit = 10)
    {
        // Prova a ottenere da cache
        $cached = $this->getCachedRecommendations($id_utente, $limit);
        if (!empty($cached)) {
            return $cached;
        }

        // Se non ci sono cache o sono scadute, genera nuove raccomandazioni
        return $this->generateRecommendations($id_utente, $limit);
    }
}